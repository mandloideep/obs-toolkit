...
function TextConfigurator() {
  ...
  const resolvedDefaults = useMemo(
    () => applyGlobalDefaults(TEXT_DEFAULTS, globalSettings),
    [globalSettings]
  )
  ...
  useEffect(() => {
    setCompanionUrl('text', previewUrl)
  ...
  const configSections = (
    <>
      ...
      {/* Section: Color (promoted from Typography for discoverability) */}
      <CollapsibleSection title="Color" defaultOpen={true} storageKey="text-color">
        <div className="grid grid-cols-2 gap-4">
      ...
      {/* Help & Guides */}
      <CollapsibleSection title="Help & Guides" defaultOpen={false} storageKey="text-help">
        <TextOverlayHelp />
      </CollapsibleSection>
    </>
  )
